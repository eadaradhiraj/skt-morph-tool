...
async fn search_dhatu_api(Path(query): Path<String>, State(state): State<Arc<AppState>>) -> Json<Value> { Json(serde_json::json!(engine::dhatu::search_dhatu(&query, &state.pool).await)) }
