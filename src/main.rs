...
async fn main() {
    ...
    let app = Router::new().nest_service("/", ServeDir::new("frontend/dist")).route("/api/analyze/:word", get(analyze_word)).route("/api/generate/verb", get(gen_verb)).route("/api/generate/participle", get(gen_participle)).route("/api/generate/declension", get(gen_declension)).with_state(Arc::new(AppState { pool }));
